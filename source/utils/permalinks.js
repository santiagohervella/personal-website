...
const trim = (string_, ch) => {
	...
	return start > 0 || end < string_.length
		? string_.substring(start, end) // eslint-disable-line unicorn/prefer-string-slice
		: string_;
};

const trimSlash = s => trim(trim(s, '/'));
const createPath = (...parameters) =>
	'/' + parameters.filter(Boolean).join('/');
